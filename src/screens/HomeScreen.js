...
import {
  ...
  Animated, StatusBar, Dimensions, Image, Linking, Alert,
} from 'react-native';
...
function ProductCard({ item, index }) {
  ...
  const runAnimation = useCallback(() => {
    Animated.parallel([
      ...
      Animated.timing(slideUp, { toValue: 0, duration: 500, delay: index * 80, useNativeDriver: true }),
    ]).start();
...
export default function HomeScreen({ navigation }) {
  ...
  useEffect(() => { runHeaderAnimation(); }, [runHeaderAnimation]);
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <Animated.View style={[styles.header, { opacity: headerFade }]}>
            ...
            <Text style={styles.sectionLabel}>ALL ITEMS — {products.length} PIECES</Text>
          </Animated.View>
        }
...
const styles = StyleSheet.create({
  ...
  profileBtnText: { color: '#f0c6c6', fontSize: 10, letterSpacing: 2, fontWeight: '500' },
  ...
  row: { justifyContent: 'space-between', paddingHorizontal: 20, marginBottom: 16 },
